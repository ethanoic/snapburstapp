...
var webapp = {
    ...
    initialize : function(mode, pgApp) {
        ...
        var cameraStreamView = React.createClass({
            ...
            componentDidMount: function() {
                this.setState({
                    video : this.refs.cameraVideo.getDOMNode(),
                    canvas : this.refs.canvasCameraView.getDOMNode(),
                    photo : this.refs.takenPhoto.getDOMNode()
                });
            },
            takePhoto: function() {
                this.state.canvas.width = width;
                this.state.canvas.height = height;
                this.state.canvas.getContext('2d').drawImage(this.state.video, 0, 0, this.state.width, this.state.height);
            ...
            render: function() {
                ...
                return (
                    <div id="cameraview" className={this.props.active ? 'no-hide' : 'hide'}>
                        ...
                        <img src="http://placekitten.com/g/200/150" ref="takenPhoto" />
                    </div>
                );
            }
        });
